...
// thanks to https://github.com/wesbos/dotfiles/blob/master/get-extensions.js
...
function getVisibleExtensions() {
  const extensions = Array.from(document.querySelectorAll(".extensions-list .monaco-list-row")).map(
    function (row) {
...
function makeMarkdownTable(extensions) {
  const rows = extensions.map((extension) => {
    return `| <a href="${makeLink(extension.id)}"><img width="100" src="${extension.icon}" alt="${
      extension.title
    }"> | <h3><a href="${makeLink(extension.id)}">${extension.title}</a></h3>${
